...
- автопрогон стратегий `orch_auto_sweep` (профили 1-4 и кастомные домены,
  ...
  режимы «до первого успеха»/«полный», пауза между стратегиями против блока
  ТСПУ (база `${Z2R_SWEEP_PAUSE:-3}` сек + добавка из запроса после режима),
  сводка, Enter=лучшая / номер / 0=возврат прежних локов, Ctrl+C с восстановлением;
