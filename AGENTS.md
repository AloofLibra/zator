...
- All zapret2 init-script invocations must go through `z2r_service_action` (`lib/config.sh`, wrapper `z2r_setsid_wrap`): it detaches the init script from the terminal's process group (setsid; on Keenetic `/usr/bin/setsid` is often absent, so it falls back to the busybox `setsid` applet via `busybox --list`, and only then to the INT/QUIT/HUP-ignored subshell). This is mandatory, not cosmetic: nfqws2 installs its own sigaction handlers for INT/TERM/HUP (`nfqws.c catch_signals`), overwriting any inherited SIG_IGN — an in-terminal-group daemon is therefore killable by Ctrl+C no matter what the shell ignores. `get_menu` re-arms an INT trap each loop iteration («Прервано (Ctrl+C) — возврат в меню») so Ctrl+C returns to the menu instead of dropping the user to the shell; `orch_auto_sweep`'s own INT trap disarms it, hence re-arming per iteration. `Entware/zapret` overrides upstream `run_daemon` (setsid spawn, upstream pidfile format `${DAEMONBASE}_N.pid`; daemon stderr goes to `/tmp/${DAEMONBASE}_N.err`, truncated on each start (harmless `seccomp:` lines are filtered out); the main menu header shows a persistent red error-count line when the log contains real errors — never print daemon errors to the terminal by timeout from the init script, it pops over the menu and confuses users; stdout stays at /dev/null as upstream) and upstream `contains` (busybox `${1#*$2}` is quadratic on the 37KB config string — ~95s per restart on mipsel; the `case`-based override is linear) — keep both overrides after `. "$EXEDIR/functions"` and in sync with upstream when it changes.
