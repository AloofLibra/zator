...
- `webui/cgi-bin/_lib.sh` has its own CGI parsing and JSON output, but intentionally reuses runtime libs (it sources `lib/config.sh`, `lib/orchestra_state.sh`, `lib/strategies.sh`, and `lib/netcheck.sh` for the shared TLS-check engine, plus `lib/actions.sh` and `lib/provider.sh` for shared setters). Keep it Bash-compatible and BusyBox/uhttpd-friendly for embedded systems. Its local `set_zapret2_init` must mirror CLI `z2r.sh` init selection: the openwrt init is taken only on real OpenWRT (probed via `/sbin/procd`; CGI has no `OSystem`) — the zapret2 tarball ships both inits on every platform, and on Keenetic/Merlin/VPS a blind file-existence check makes the start/stop buttons fail with «Не удалось выполнить команду zapret2» (guard in `tests/webui_settings_smoke.sh`).
...
- busybox-sh совместимость `lib/deploy.sh`: статический grep на bash-измы
  (`${var^^}`, `BASH_SOURCE`, RETURN-ловушки, массивы, …) плюс живой прогон
  всего файла под `busybox sh` до usage-выхода, если busybox есть в PATH
  (на роутерах Entware/OpenWRT — всегда); `sh -n` у busybox подстановки не
  проверяет, поэтому исполняется сам файл.
