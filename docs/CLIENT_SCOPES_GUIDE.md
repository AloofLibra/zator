...
```text
23. Client scopes (Beta): … Сейчас: [включен/выключен]
```

— переключатель режима.

«Управление стратегиями» (пункт 1 главного меню):

- при выключенном режиме открывается сразу список профилей, как раньше;
- при включённом — сначала выбор клиента: `default` (все клиенты),
  существующий mark, «Новый клиент (IP → mark)» или «Удалить клиента»;
  затем привычное меню уже в контексте выбранного клиента.

## 3. Настройка IP клиента

При добавлении клиента IP вводится вручную или выбирается из списка
найденных устройств (ARP-таблица LAN-моста: Keenetic — br0, OpenWRT —
br-lan). Уже привязанные к mark IP в списке не показываются.

Mark: Enter — следующий свободный (автоназначение начинается с 2), или
номер существующей группы — тогда IP добавляется в неё (несколько
устройств = одна марка = общие стратегии). **mark:1 зарезервирован**
под собственный трафик роутера (см. раздел 9), mark:0 использовать
нельзя (0 = «метки нет»).
...
## 9. Роутер как отдельный клиент (дизайн на будущее)

Собственный трафик роутера (тесты `check_access`, будущий автоперебор по cron)
планируется выносить в отдельный mark. Почему это важно:

- автоподбор по расписанию сможет гонять проверки в контексте «роутерного»
  mark, не затрагивая стратегии реальных клиентов;
- найденное лучшее примеряется изолированно, глобальная смена выполняется
  только после подтверждения результата;
- per-mark файлы позволяют хранить пробные стратегии отдельно и чистить
  их одним файлом.

Сейчас это не реализовано: маркируются только LAN-клиенты (mangle/PREROUTING);
разметка собственного трафика роутера потребует mangle/OUTPUT. Решение
зафиксировано здесь, чтобы не ломать структуру scope-файлов при развитии.

Домены работают так же: попадание в лист TCP_Custom — глобальное действие
(обработка у всех сразу), а подобранная для домена стратегия закрепляется
за выбранным клиентом и живёт в его per-mark файле.
